...
if ($Apply.IsPresent) {
    ...
    foreach ($dir in @($specsRoot, $resultsRoot, $testsRoot, $toolsRoot, $memoryRoot)) {
...
if (-not $NoReport.IsPresent) {
    ...
    $lines.Add("- Retention days: specs=$RetainSpecsDays results=$RetainResultsDays tools=$RetainToolsDays tests=$RetainTestsDays")
    $lines.Add('')
